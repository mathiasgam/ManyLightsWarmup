...
template<class T, class V, unsigned int K, unsigned int N>
class API ArithVecArray {
public:
...
protected:
	...
	ArithVecArray(T _a, T _b, T _c, T _d) :
		data{ _a, _b, _c, _d }
	{
		assert(K == 4);
	}
...
public:
	...
	inline V operator * (const ArithVecArray<T, V, K> &other) const {
		V res;
		for (int i = 0; i < K; i++) {
...
inline V operator*(const T& c, const ArithVecArray<T, V, K>& v) {
	V res;
	for (int i = 0; i < K; i++) {
		res[i] = c * v[i];
	}
...
template <class T, class V, unsigned int K>
inline T dot(const ArithVecArray<T, V, K>& v0, const ArithVecArray<T, V, K>& v1) {
	return (v0*v1).element_sum();
}


template <class T, class V, unsigned int K>
inline T sqr_length(const ArithVecArray<T, V, K>& v) {
	return (v*v).element_sum();
}

template <class T, class V, unsigned int K>
inline V abs(const ArithVecArray<T, V, K>& v) {
	V res;
	for (int i = 0; i < K; i++) {
		res[i] = v[i] < 0 ? -v[i] : v[i];
	}
...
template<class T, class V, unsigned int K>
inline V min(const ArithVecArray<T, V, K>& v0, const ArithVecArray<T, V, K>& v1) {
	V v;
	for (int i = 0; i < K; i++) {
...
template<class T, class V, unsigned int K>
inline V max(const ArithVecArray<T, V, K>& v0, const ArithVecArray<T, V, K>& v1) {
	V v;
	for (int i = 0; i < K; i++) {
		v[i] = v0[i] > v1[i] ? v0[i] : v1[i];
	}
...
template <class T, class V, unsigned int K>
inline std::ostream& operator<<(std::ostream&os, const ArithVecArray<T, V, K>& v)
{
	os << "[ ";
